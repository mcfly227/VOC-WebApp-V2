...
def _tab8(ws, db):
    ...
    for j, v in enumerate(["Product Number", "Lbs/gal", "Automotive or non-Automotive"], start=1):
        ws.cell(row=5, column=j, value=v)
    row = 6
    for p in db.query(models.Product).filter(models.Product.as_applied_voc.isnot(None)) \
              .order_by(models.Product.number):
        ws.cell(row=row, column=1, value=p.number)
        c = ws.cell(row=row, column=2, value=round(p.as_applied_voc, 4))
